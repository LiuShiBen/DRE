    ...
    with torch.no_grad():
        for i, (imgs, fnames, pids, cids, domains) in enumerate(data_loader):
            ...
            for fname, feature, pid, cid in zip(fnames, features, pids, cids):
                feature0 = [feature[:768], feature[768:768 * 2], feature[768 * 2:768 * 3]]
...
def initial_classifier(model, data_loader):
    ...
    pid2features2 = collections.defaultdict(list)
    pid2features3 = collections.defaultdict(list)
    ...
    for feature, pid in zip(features_all, labels_all):
        ...
        pid2features2[pid].append(feature[1])
        pid2features3[pid].append(feature[2])
    ...
    class_centers = (F.normalize(class_centers1, dim=1).float() + F.normalize(class_centers2, dim=1).float() +
                     F.normalize(class_centers3, dim=1).float()) / 3
...
def select_replay_samples(model, dataset, training_phase=0, add_num=0, old_datas=None, select_samples=2):
    ...
    for feature, pid, fname, cid in zip(features_all, labels_all, fnames_all, camids_all):
        # print()
        features = (feature[0] + feature[1] + feature[2]) / 3
