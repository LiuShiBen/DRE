...
def main_worker(args):
    ...
    # Create model
    model = build_vit_backbone(num_class=num_classes_dukemtmc)
    model.cuda()
    ...
    initclassifier(model, num_class=[num_classes_dukemtmc, num_classes_msmt17], data="msmt17",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch == args.epochs - 1):
            ...
            for evaluator, name, test_loader in zip(evaluators, names, test_loaders):
                cmc, mAP_msmt17, _, _ = eval_func(epoch, evaluator, model, test_loader, name, old_model)

            save_checkpoint({
                'state_dict': model.state_dict(),
                'epoch': epoch + 1,
            }, True, fpath=osp.join(args.logs_dir, 'current_checkpoint_step_2.pth.tar'))
            print('Finished epoch {:3d}  msmt17 mAP: {:5.1%} '.format(epoch, mAP_msmt17))
    ...
    # Expand the dimension of classifier
    initclassifier(model, num_class=[num_classes_dukemtmc, num_classes_msmt17, num_classes_market], data="market",
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if (epoch == args.epochs - 1):
            ...
            print('Finished epoch {:3d}  DukeMTMC mAP: {:5.1%}'.format(epoch, mAP_market))
    ...
    initclassifier(model,
                   num_class=[num_classes_dukemtmc, num_classes_msmt17, num_classes_market, num_classes_cuhksysu],
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if epoch == args.epochs - 1:
            ...
            print('Finished epoch {:3d}  MSMT17 mAP: {:5.1%}'.format(epoch, mAP_cuhksysu))
