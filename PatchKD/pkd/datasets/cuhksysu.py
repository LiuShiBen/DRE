...
from pkd.data.datasets.ps_dataset import pickle, unpickle
import numpy as np
import torch
...
class IncrementalSamples4cuhksysu(IncrementalPersonReIDSamples):
    ...
    def __init__(self, datasets_root, relabel=True, combineall=False):
        ...
        self.dataset_dir = osp.join(self.root, 'cuhksysu')
        ...
        if all(map(osp.exists, required_files_state)):
            ...
            gallery = self.process_dir(self.processed_dir_gallery, relabel=False, gallery=True)
        else:
    ...
    def process_dir(self, dir_path, relabel=False, gallery=False):
        ...
        for img_path in img_paths:
            ...
            if relabel:
                pid = pid2label[pid]
            data.append((img_path, pid, 1 if gallery else 0, 'cuhksysu', pid))
