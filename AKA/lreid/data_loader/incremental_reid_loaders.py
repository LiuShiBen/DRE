...
from lreid.datasets import (IncrementalSamples4subcuhksysu, IncrementalSamples4market,
                               ...
                               IncrementalSamples4mix, IncrementalSamples4occduke,
                               IncrementalSamples4occreid
                            )
...
class IncrementalReIDLoaders:
    ...
    def _get_test_samples(self, a_test_dataset):
        if a_test_dataset == 'market':
            ...
            query, gallery = samples.query, samples.gallery
        elif a_test_dataset == 'occduke':
            samples = IncrementalSamples4occduke(self.config.datasets_root, relabel=True, combineall=self.config.combine_all)
            query, gallery = samples.query, samples.gallery
        elif a_test_dataset == 'occreid':
            samples = IncrementalSamples4occreid(self.config.datasets_root, relabel=True, combineall=self.config.combine_all)
            print("....:")
            query, gallery = samples.query, samples.gallery
