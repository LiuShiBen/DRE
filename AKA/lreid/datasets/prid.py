...
class IncrementalSamples4prid(IncrementalPersonReIDSamples):
    ...
    def process_split(self, split):
        ...
        for pid in train_pids:
            ...
            train.append((img_a_path, pid, 0, 'prid', pid))
            img_b_path = osp.join(self.cam_b_dir, img_name)
            train.append((img_b_path, pid, 1, 'prid', pid))
        ...
        for pid in test_pids:
            img_name = 'person_' + str(pid).zfill(4) + '.png'
            img_a_path = osp.join(self.cam_a_dir, img_name)
            query.append((img_a_path, pid, 0, 'prid', pid))
            img_b_path = osp.join(self.cam_b_dir, img_name)
            gallery.append((img_b_path, pid, 1, 'prid', pid))
