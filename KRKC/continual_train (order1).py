...
def main_worker(args):
    ...
    #print("dataset.images_dir:", dataset_market.images_dir, dataset_cuhksysu.images_dir)
    # Create model
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        trainer.train(epoch, train_loader_market, None, optimizer, old_optimizer=None, training_phase=1,
    ...
    model.module.classifier = nn.Linear(2048, num_classes_market + num_classes_cuhksysu, bias=False)
    model.cuda()
    ...
    evaluator_cuhksysu = R1_mAP_eval(len(dataset_cuhksysu.query), max_rank=50, feat_norm=True)
    evaluators.append(evaluator_cuhksysu)
    ...
    trainer = Trainer(model, num_classes_market + num_classes_cuhksysu, margin=args.margin)

    for epoch in range(start_epoch, args.epochs):

        train_loader_cuhksysu.new_epoch()
        trainer.train(epoch, train_loader_cuhksysu, replay_dataloader, optimizer, old_optimizer, training_phase=2,
    ...
    replay_dataloader, cuhksysu_replay_dataset = select_replay_samples(model, dataset_cuhksysu, training_phase=2,
    ...
    # Expand the dimension of classifier
    org_classifier_params = model.module.classifier.weight.data
    model.module.classifier = nn.Linear(2048, num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc, bias=False)
    model.module.classifier.weight.data[:(num_classes_market + num_classes_cuhksysu)].copy_(org_classifier_params)
    model.cuda()
    add_num = num_classes_market + num_classes_cuhksysu

    # Initialize classifer with class centers
    class_centers = initial_classifier(model, init_loader_dukemtmc)
    model.module.classifier.weight.data[(num_classes_market + num_classes_cuhksysu):].copy_(class_centers)
    model.cuda()
    ...
    trainer = Trainer(model, num_classes_dukemtmc + add_num, margin=args.margin)

    for epoch in range(start_epoch, args.epochs):

        train_loader_dukemtmc.new_epoch()
        trainer.train(epoch, train_loader_dukemtmc, replay_dataloader, optimizer, old_optimizer, training_phase=3,
                      train_iters=len(train_loader_dukemtmc), add_num=add_num, old_model=old_model, replay=True)
        lr_scheduler.step()
        old_lr_scheduler.step()
    ...
    replay_dataloader, dukemtmc_replay_dataset = select_replay_samples(model, dataset_dukemtmc, training_phase=3,\
                                                   add_num=add_num, old_datas=cuhksysu_replay_dataset)
    #model space consolidation
    model = model.module
    old_model = old_model.module
    alpha = 1/3
    ...
    org_classifier_params = model.module.classifier.weight.data
    model.module.classifier = nn.Linear(2048, num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc + num_classes_msmt17, bias=False)
    model.module.classifier.weight.data[:(num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc)].copy_(org_classifier_params)
    model.cuda()
    add_num = num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc

    # Initialize classifer with class centers
    class_centers = initial_classifier(model, init_loader_msmt17)
    model.module.classifier.weight.data[(num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc):].copy_(class_centers)
    model.cuda()
    ...
    replay_dataloader, msmt17_replay_dataset = select_replay_samples(model, dataset_msmt17, training_phase=4, \
                                                                       add_num=add_num,
                                                                       old_datas=dukemtmc_replay_dataset)
    # model space consolidation
    model = model.module
    old_model = old_model.module
    alpha = 1 / 4
    ...
    model.module.classifier = nn.Linear(2048,
                                        num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc + num_classes_msmt17 + num_classes_cuhk03,
                                        bias=False)
    model.module.classifier.weight.data[:(num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc + num_classes_msmt17)].copy_(
        org_classifier_params)
    model.cuda()
    add_num = num_classes_market + num_classes_cuhksysu + num_classes_dukemtmc + num_classes_msmt17
